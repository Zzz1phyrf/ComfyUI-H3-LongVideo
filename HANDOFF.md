...
- 项目默认参考图区已取消展开/收起按钮，始终直接显示；因此分段的“参考图来源 / 本段画面 / 手写提示词”不会再看起来受上方展开状态控制。
...
- 尚未在重启后的真实浏览器中人工点击验收本轮的始终显示与自动迁移。
...
5. 验收项目默认参考图区始终显示、新项目分段素材控件立即可见，以及空白画布上传图片不再走 `ref_image` 接线报错；截图顶部曾出现的 `ValueError 4 H3LVPromptExpand` 仍需在新进程中取得完整控制台错误与复现步骤。
