...
5. 继续验收项目默认参考图折叠与继承状态；截图顶部曾出现的 `ValueError 4 H3LVPromptExpand` 仍需在新进程中取得完整控制台错误与复现步骤。
