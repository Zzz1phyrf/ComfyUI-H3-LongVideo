...
1. 推荐打开“一体化”工作流上传歌曲，选择 singing 或 speaking。参考图直接连接官方 MiniMax H3 节点；长视频节点不接图、不识图，也不保存或转发图片。歌手模式可选择镜头活跃度和最远景别；表演强度按分段测量自动安排。语音识别默认使用当前 ComfyUI Python，并从 `ComfyUI/models/faster-whisper` 自动发现完整模型快照；技术设置里的两个路径只用于覆盖默认值。可接已有分离节点的人声；未接时只尝试已缓存的 HDEMUCS 模型，不自动下载。
...
3. 提示词小助手的“用户提示词”是可选素材说明：需要精确指定图1、图2或人物与音频关系时再填写；留空时只扩写当前运镜和表演，不猜参考图内容。确认并关闭审核后，再点击 ComfyUI 自带的整张工作流“运行”，插件会接管同一工作流并从第1段开始顺序循环生成；此时若只运行 H3 长视频节点，仍视为重新分析并创建新项目。审核面板中的绿色“开始顺序生成”是整张工作流运行的快捷入口。连线错误仍会直接拦截。
